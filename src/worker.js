...
export default {
    async fetch(request) {
        try {
            ...
            反代IP = 反代IP ? 反代IP : request.cf.colo + '.koukouIP.cmliuSSSS.NET';
            ...
            async function retry() {
                try {
                    ...
                    if (enableSocks === 'ppou5') {
                        ...
                        tcpSocket = await connect({ hostname: koukouIP, port: koukouPort }, { allowHalfOpen: true });
                    }
...
async function httpConnect(addressType, addressRemote, portRemote, parsedSocks5Address) {
    ...
    if (username && password) {
        ...
        connectRequest += `koukou-Authorization: Basic ${base64Auth}\r\n`;
    }

    connectRequest += `User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36\r\n`;
    connectRequest += `koukou-Connection: Keep-Alive\r\n`;
...
async function 解析地址端口(koukouIP) {
    koukouIP = koukouIP.toLowerCase();
    let 地址 = koukouIP, 端口 = 443;
    if (koukouIP.includes('.tp')) {
        const tpMatch = koukouIP.match(/\.tp(\d+)/);
        if (tpMatch) 端口 = parseInt(tpMatch[1], 10);
        return [地址, 端口];
    }
    if (koukouIP.includes(']:')) {
        const parts = koukouIP.split(']:');
        地址 = parts[0] + ']';
        端口 = parseInt(parts[1], 10) || 端口;
    } else if (koukouIP.includes(':') && !koukouIP.startsWith('[')) {
        const colonIndex = koukouIP.lastIndexOf(':');
        地址 = koukouIP.slice(0, colonIndex);
        端口 = parseInt(koukouIP.slice(colonIndex + 1), 10) || 端口;
    }
    return [地址, 端口];
}
...
async function 反代参数获取(request) {
    ...
    启用ssty全局反代 = searchParams.has('globalkoukou') || false;

    
    const koukouMatch = pathLower.match(/\/(koukouip[.=]|pyip=|ip=)(.+)/);
    if (searchParams.has('koukouip')) {
        const 路参IP = searchParams.get('koukouip');
        反代IP = 路参IP.includes(',') ? 路参IP.split(',')[Math.floor(Math.random() * 路参IP.split(',').length)] : 路参IP;
        return;
    } else if (koukouMatch) {
        const 路参IP = koukouMatch[1] === 'koukouip.' ? `koukouip.${koukouMatch[2]}` : koukouMatch[2];
        反代IP = 路参IP.includes(',') ? 路参IP.split(',')[Math.floor(Math.random() * 路参IP.split(',').length)] : 路参IP;
        return;
    }
