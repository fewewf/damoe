const _LZYatphGWKPqCyCo = '276c88bd-165f-4361-aef7-f58f490d7dc3'; // 建议修改为自己的规范化UUID，如不需要可留空 @GoodLiux优化版
let _iHNsBJIspPLKvKPk = '',
...
async function _jXfxVuvsOQsZnbrm(_aRsBwlhwKeDpxXtT, _xHOMVbLumNwUKErn, _OTtEInlHayZlUIpn, _gyoWTTnIspfRmhus = null, _jAxipFuctmgLRCEi = 0) {
  const _IzJgVawVozZSahgX = 8; // 最大重试8次
  const _JdDhFJPQEsRtlrqf = 128 * 1024; // 单帧最大 128 KB
  const _SReOEvBXXijDWoxq = 2 * 1024 * 1024; // 最大缓存 2 MB
  const _nMLamCPIAycyPvrK = 10; // ms，定期 flush
  const _XxxdGvBtKBfuxrAS = 200; // ms，初始重试延迟
...
async function _WhQhlONCWPVxkgLD(_spLKrYXkNoeZNPyW) {
  ...
  if (_HQtdeJoTRYeAmeto = _SNeyTjjrmxXQahpN.match(/\/(socks5?|http):\/?\/?(.+)/i)) {
    ...
    _ImrDOcpEklzmKNnc = _PPgDPvHwagjgLmdP.startsWith('g') || _ImrDOcpEklzmKNnc; // gs5 或 ghttp 开头启用全局
  }
